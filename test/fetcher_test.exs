defmodule MozartFetcher.FetcherTest do
  ...
  test "it gets success when passing component config" do
    assert Fetcher.process([%Config{endpoint: "http://localhost:8082/foo", id: "foo"}]) ==
             ~s({"components":[{"id":"foo","index":0,"status":200,"envelope":{"head":[],"bodyInline":"","bodyLast":[]}}]})
  end
  ...
  test "it returns envelopes for components timeout" do
    ...
    expected =
      ~s({"components":[{"id":"foo","index":0,"status":200,"envelope":{"head":[],"bodyInline":"","bodyLast":[]}},{"id":"timeout","index":1,"status":408,"envelope":{"head":[],"bodyInline":"","bodyLast":[]}}]})
  ...
  test "it returns empty envelopes with status and id for components erroring out with no generated response" do
    ...
    expected =
      ~s({"components":[{"id":"foo","index":0,"status":200,"envelope":{"head":[],"bodyInline":"","bodyLast":[]}},{"id":"timeout","index":null,"status":408,"envelope":{"head":[],"bodyInline":"","bodyLast":[]}}]})
